...
## 十一、待办事项（2026-08-11 更新）

### 明日（国信日限额 0 点重置后）补跑清单

```bash
python3 src/sync_financial.py    # 财务：000534 起 24 只刷新 + 301707 骨架行填值
python3 src/sync_extra.py        # 宏观 9 指标 + 资金流向 6 只
python3 src/sync_daily.py --full # 补 2 条超时宏观文章（顺带全量刷新）
```

背景：2026-08-11 全量同步（09:56）消耗当日国信配额后，`sync_financial.py` 仅完成 000037 即触发
197006 熔断（熔断封装按设计生效，未空转）。财务最新完整期仍为 2026Q1。

### 观察项
- 预测闭环首轮真实结算（2026-08-10 到期 24 条）：命中 5/24（20.8%），样本量小属正常；
  观察 2~4 周后再评估，随 prediction_log 积累由二项检验逐步过滤噪声模式
